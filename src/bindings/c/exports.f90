 ...
 group_name_len, mesh_type, mesh_type_len, grav_dirn, grav_factor, bc_type,&
  bc_type_len, inlet_bc, outlet_bc,L_p, sigma, pi_c, pi_alv, c_L)&
   bind(C, name="export_parameters_edema_c")
    ...
    use iso_c_binding, only: c_ptr
    use utils_c, only: strncpy
    use exports, only: export_parameters_edema
    use other_consts, only: MAX_STRING_LEN, MAX_FILENAME_LEN
    implicit none

    integer,intent(in) :: filename_len, group_name_len, mesh_type_len, bc_type_len
    type(c_ptr), value, intent(in) :: PARAMFILE, group_name, mesh_type, bc_type
    character(len=MAX_FILENAME_LEN) :: filename_f
    character(len=MAX_STRING_LEN) :: group_name_f, mesh_type_f, bc_type_f
    ...
    call strncpy(filename_f, PARAMFILE, filename_len)
    call strncpy(group_name_f, group_name, group_name_len)
...
!
  subroutine export_terminal_starling_variables_c(EXNODEFILE, filename_len, name, name_len)&
    ...
    use iso_c_binding, only: c_ptr
    use utils_c, only: strncpy
    use exports, only: export_terminal_starling_variables
    use other_consts, only: MAX_STRING_LEN, MAX_FILENAME_LEN
    implicit none
    integer,intent(in) :: filename_len, name_len
    type(c_ptr), value, intent(in) :: EXNODEFILE, name
...
#if defined _WIN32 && defined __INTEL_COMPILER
    call so_export_terminal_starling_variables(filename_f, name_f)
#else
    call export_terminal_starling_variables(filename_f, name_f)
#endif

  end subroutine export_terminal_starling_variables_c
